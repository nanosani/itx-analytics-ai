...
| `submit_for_indexing` | `paths` or `bulk: "bing_missing"`, `engine` (auto/bing/indexnow) | Write: ask Bing (Webmaster API within its daily quota, or IndexNow) to crawl pages. Google has no API — rows from `get_index_status` carry `inspect_url` for Search Console's "Request indexing". |
