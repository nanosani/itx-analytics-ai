...
| `get_index_status` | `path`, `google` (indexed/not_indexed/unknown), `bing` (indexed/crawled/not_known/error/unknown), `limit` | Indexing on Google and Bing for every published page: coverage per engine with Google's reasons, problem lists (not indexed, new pages waiting, blocked by robots/noindex, other canonical chosen, fetch/crawl errors, on one engine only), Bing crawl trend and crawl issues; one page with `path`; a filtered page list with `google`/`bing`. Impressions in the last 90 days count as proof of indexing; URL Inspection and Bing URL info cover the rest daily. |
